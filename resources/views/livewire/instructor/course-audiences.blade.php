<section>
    <h1 class="text-2xl font-bold mb-8">Audiencia</h1>
            ...
            @if ($audience->id == $item->id)
                <header class="flex justify-between items-center px-4 py-3">
                    <form class="flex-1" wire:submit.prevent="update">
                        <input wire:model="audience.name" type="text" class="form-input @if($errors->has('audience.name')) invalid @endif" placeholder="Escribir...">
                        @error('audience.name')
    ...
    <div class="mt-4" x-data="{open: false}">
        <div class="text-center mb-3">
            <button class="btn btn-green" type="button" @click="open = true; if (open) $nextTick(()=>{ $refs.nameInput.focus() });" x-show="!open">
                ...
                </svg> Agregar nueva audiencia
            </button>
        </div>
        <article class="bg-gray-50 rounded shadow px-4 py-3 mb-3" x-show="open">
            <h1 class="text-lg font-bold mb-3">Nueva audiencia</h1>
